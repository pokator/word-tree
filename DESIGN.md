...
- **Node coloring:** the root node and any selected node always carry `--accent`. Kanji and word nodes share a single restrained `--node-neutral` (light `#8a8371`, dark `#6b7182`) — they're already distinguishable by shape, size, and label content (a single kanji glyph vs. a multi-character word), so a second and third saturated hue here only adds competing signal as the graph grows. This directly replaces the old per-type rainbow (light: slate blue + forest green; dark: neon pink + cyan + amber).
- **Kanji-position links (start/middle/end):** kept as a genuine semantic signal but turned down from a competing full-saturation line (opacity 0.8, 2px) to a quiet hint (opacity 0.55, 1.75px) — visible on inspection, not fighting the accent for attention.
